...
def load_jigsaw_raw(normalize):
  ...
  for entry in train_rows:
    ...
    toxicity = "Toxic" if entry["toxicity"] >= 0.5 else "Not toxic"
    gender = "M" if entry["male"] > entry["female"] else "F"
    res.append([comment_text, toxicity, gender])

  np.save('jigsaw_train.npy', np.array(res))
  ...
  np.save('jigsaw_test.npy', np.array(res))

  train_data = np.load('jigsaw_train.npy', allow_pickle=True)
  test_data = np.load('jigsaw_test.npy', allow_pickle=True)

  X , X_dev, X_test = train_data[:10000, 0], train_data[10000: 12500, 0], test_data[:, 0]
  Y, Y_dev, Y_test = train_data[:10000, 1], train_data[10000: 12500, 1], test_data[:, 1]
...
if __name__ == "__main__":
 ...
 if mode == "glove":
     ...
     X,Y,X_dev,Y_dev, X_test, Y_test= load_jigsaw_raw(normalize=True)
 ...
 for i in range(4,20):
         ...
         os.makedirs("interim/jigsaw{}/kernel/projected".format(i), exist_ok=True)
         os.makedirs("interim/jigsaw{}/kernel/nystrom".format(i), exist_ok=True)
         os.makedirs("interim/jigsaw{}/kernel/preimage".format(i), exist_ok=True)
         os.makedirs("interim/jigsaw{}/kernel/multiple".format(i), exist_ok=True)
